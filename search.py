...
def graph_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            return node, node.path_cost, visited, generated
...
def branch_and_bound_graph_search(problem):
    return graph_search(problem, OrderedQueue(lambda node:node.path_cost))

def branch_and_bound_with_subestimation_graph_search(problem):
    return graph_search(problem, OrderedQueue(lambda node:node.path_cost+problem.h(node)))

def branch_and_bound_with_overestimation_graph_search(problem):
    return graph_search(problem, OrderedQueue(lambda node:node.path_cost+10*problem.h(node)))
